...
- **Bump `versionCode`** in `android/app/build.gradle.kts` (currently **63** / 1.4.13) before EVERY APK build — the WebView caches the PWA service worker; `MainActivity.clearCacheOnVersionChange()` clears it on a version bump. Forgetting = users stuck on the old UI.
