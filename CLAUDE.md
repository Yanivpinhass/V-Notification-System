...
- **Room schema changes can SILENTLY WIPE ALL USER DATA if done wrong.** Any `@Entity` change requires: bump `@Database(version=N)` in `android/.../db/MagavDatabase.kt` (currently **9**), add `MIGRATION_N_(N+1)`, and register it in BOTH `addMigrations(...)` sites in `MagavApplication.kt` (latest: `MIGRATION_8_9` adds the `CallbackConfig` table — additive-only, entity/migration defaults verified identical via `exportSchema`). `initializeDatabase()` (`android/.../MagavApplication.kt:102-137`) deliberately omits `fallbackToDestructiveMigration` and recovers ONLY on SQLCipher key/corruption errors — every other error (incl. a forgotten-migration schema-hash mismatch) is re-thrown so it crashes VISIBLY instead of wiping data. Verify on a populated dev device before shipping. [ADR-004 — born from a real data-loss incident]
...
- **Bump `versionCode`** in `android/app/build.gradle.kts` (currently **76** / 1.4.26) before EVERY APK build — the WebView caches the PWA service worker; `MainActivity.clearCacheOnVersionChange()` clears it on a version bump. Forgetting = users stuck on the old UI.
...
- **Auto-callback-to-gate (Android-only, `android/.../callback/`).** An eligible unanswered incoming call (volunteer on a today/yesterday non-canceled shift, feature active, inside the Israel-tz window) is rejected and the configured Gate number auto-dialed after 20s. Detection = manifest `PHONE_STATE` + `READ_CALL_LOG` (NOT `CallScreeningService`); the 20s wait = a SINGLE one-shot `setExactAndAllowWhileIdle` alarm; reject/dial via `TelecomManager.endCall()`/`placeCall()` (needs `CALL_PHONE`+`ANSWER_PHONE_CALLS`, NOT `ACTION_CALL`). Correctness rests on ONE fire-time `callState==RINGING` re-check (no state machine); busy line ⇒ do nothing. The **All callers** toggle bypasses the volunteer filter (any caller); Active + window still apply. Config is the `CallbackConfig` singleton + the **Android-Ktor-only** `GET/PUT /api/callback-config` (no .NET counterpart — another accepted [ISS-004] divergence, like `/api/settings/sms-sim`); the React `CallbackSettingsPage` is gated to the Android WebView via `window.NativeMedia`. Decoupled from SMS — touches no SMS file or `MagavServerService`. [ADR-021]
...
- Why decisions were made (21 ADRs + Knowledge Log) → `.ai/docs/decisions.md`
...
- **Known issues** → `.ai/docs/issues.md`. As of 2026-06-30 (auto-callback-to-gate, uncommitted): **2 open** — the hardcoded `MagavConstants.PasswordKey` encryption-key constant [ISS-007, narrowed] and the root `.gitignore` `db/` rule (line 41) that silently git-ignores NEW Android `db/`-package source files [ISS-010, new — the callback `CallbackConfigEntity.kt`/`CallbackConfigDao.kt` need `git add -f`]; **2 accepted-by-design** — the triplicated contract [ISS-004, now also the Android-only `/api/callback-config`] and the Android `Volunteer` divergence [ISS-003]; **5 resolved** (public SMS-approval route, orphan page, `Results.Problem`, swallow-all dedup, dead password validator). The auto-callback feature added no defect in its own code (fail-safe, well-guarded); the Duty Log and device-allowlist gate likewise added no formal finding.
<!-- DEEPINIT:END -->
