...
- **Main flow:** EITHER fill `DutyLogPage` (date / start+end time / shift-name preset-or-free-text / vehicle / volunteer multi-select ∪ free-text people; ≥1-person validation) OR click `צור יומן הפעלה` on a Shifts team card (maps the in-memory group → **default** 19:00–02:00, `carId` verbatim, unresolved rows dropped) → both call `openPreview(DutyLogData)` (provider rendered above `AdminLayout`) → fit-to-screen / pinch-zoom preview (a `שנה שעות` toggle can override the hours live — the edited `effectiveData` feeds BOTH the report and the export, `778a2dd`) → `exportDutyLogPng(effectiveData)` lazy-loads html2canvas, renders the 1123px RTL `DutyLogReport` off-screen in its own React root, awaits fonts+emblem, rasterizes to PNG → **desktop:** `<a download>`; **Android WebView:** `window.NativeMedia.saveImageToGallery` (MediaStore `Pictures/Magav`) + `shareImage` (FileProvider `ACTION_SEND`). [HIGH] (`web/client/src/features/duty-log/*`, `pages/DutyLogPage.tsx`, `pages/ShiftsManagementPage.tsx:707-719`; Android `media/MediaBridge.kt`) — **no DB writes, no new endpoints, no contract change** (ADR-019).
- **Alternates:** `endTime ≤ startTime` ⇒ end date +1 day (shared `deriveEndDate`); 0 resolved people ⇒ the Shifts button is hidden (form path requires ≥1); `toBlob` null ⇒ Hebrew error toast; Android save failure ⇒ error toast, share still attempted; an invalid/cleared hour in the preview editor disables export until valid. [HIGH]

### UC-010 — A device is admitted/blocked at Android app launch (device allowlist) — ANDROID-ONLY
- **Actor:** Any person launching the Android app on a physical device. [HIGH]
- **Goal:** Restrict the distributed APK to explicitly-approved devices.
- **Preconditions:** Embedded Ktor server is up (`GET /api/health` ready) and the `LICENSE_PHONES`/expiry license gate passed.
- **Main flow:** `MainActivity.waitForServerAndLoad()` runs an ordered gate chain — health → `LicenseValidator.isValid()` → **`DeviceAllowlist.isAllowed()`**. The allowlist compares this device's `Settings.Secure.ANDROID_ID` (trimmed+lowercased) against the hardcoded `DeviceAllowlist.ALLOWED` set; on a match → `handleSessionAwareStartup()` loads the app. [HIGH] (`MainActivity.kt:185-197`, `license/DeviceAllowlist.kt:22-41`)
- **Alternates / failure:** unreadable/blank ANDROID_ID OR id not in the set (incl. an **EMPTY set ⇒ ALL devices blocked**) ⇒ the Hebrew block page `buildDeviceBlockHtml(deviceId)` renders the device's id + a copy button wired to `window.NativeClip.copyDeviceId()` (so the user can send it for approval); the app never loads. The gate is **fail-CLOSED** and ANDROID_ID is keystore-scoped, so a signing-key change re-locks every device (WA-009). [HIGH] (`MainActivity.kt:191-194,299`, `license/DeviceClipboardBridge.kt`)
- **Web target:** N/A — this gate exists only on Android (web is served over Nginx; no per-device gate). [HIGH]
...
| **WA-003** | **`versionCode` must be bumped on every APK build** to invalidate the WebView's cached PWA/service-worker. | `MainActivity.clearCacheOnVersionChange()` clears the WebView cache only when the stored versionCode differs; forgetting to bump leaves users on a stale cached React UI. Current `versionCode=75` / `1.4.25`. | `android/.../MainActivity.kt:295`, `android/app/build.gradle.kts:16`; CLAUDE.md | HIGH |
