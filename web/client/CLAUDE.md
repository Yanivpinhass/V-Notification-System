...
- **The documented public SMS-approval page is UNWIRED** — `App.tsx:22-25` registers only `/` and `*`; `VolunteerSmsApprovalPage.tsx`/`RevokeSmsApprovalPage.tsx` are imported nowhere (and `RevokeSmsApprovalPage.tsx:33` calls a nonexistent `volunteersService.revokeSmsApproval`). The server endpoint exists, but the React flow is dead. [ISS-001/002]
